...
export default function CouponSuggestions({ onSelect, subtotal }: CouponSuggestionsProps) {
  ...
  return (
    <div className="mt-4 p-4 bg-[#F3F5F7] rounded-xl border border-[#E8ECEF] animate-in fade-in slide-in-from-top-2 duration-300">
      <div className="flex items-center gap-2 mb-3">
        ...
        <h4 className="text-sm font-semibold text-[#141718]">Available Offers</h4>
      </div>
            ...
            >
              <div className="flex items-center justify-between w-full mb-1">
                <div className="flex items-center gap-2">
                  <div className={`p-1 rounded ${isEligible ? "bg-[#38CB89]/10 text-[#38CB89]" : "bg-gray-200 text-gray-500"}`}>
                    <Ticket size={14} />
                  </div>
                  <span className="font-bold text-[14px] tracking-tight">{coupon.code}</span>
                </div>
                <span className={`text-[13px] font-bold ${isEligible ? "text-[#38CB89]" : "text-gray-400"}`}>
              ...
              {!isEligible && (
                <div className="flex items-center gap-1 mt-1 text-[11px] text-[#6C7275]">
                  <AlertCircle size={12} />
                  <span>Add ${ (coupon.min_order_amount - subtotal).toFixed(2) } more to unlock</span>
                </div>
              )}
